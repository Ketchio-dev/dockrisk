...
const LINE = "#3987e5"; // series-1, dark surface (validated reference palette)
  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H}`} className="h-24 w-full" onMouseLeave={() => setHover(null)}
        ...
        {[0, 40, 80].map((v) => (
          <g key={v}><line x1={PL} x2={W - 6} y1={y(v)} y2={y(v)} stroke="#334155" strokeWidth="1" strokeDasharray="2 4" />
            <text x={PL - 4} y={y(v) + 3} fontSize="9" textAnchor="end" fill="#64748b">{v}</text></g>
        ))}
        <path d={d} fill="none" stroke={LINE} strokeWidth="2" strokeLinejoin="round" />
        {s.length > 0 && [0, Math.floor(s.length / 2), s.length - 1].map((i) => <text key={i} x={x(i)} y={H - 4} fontSize="9" textAnchor={i === 0 ? "start" : i === s.length - 1 ? "end" : "middle"} fill="#64748b">{s[i].t}</text>)}
        {h && hover != null && (<g><line x1={x(hover)} x2={x(hover)} y1={PT} y2={H - PB} stroke="#94a3b8" strokeWidth="1" /><circle cx={x(hover)} cy={y(h.kmh)} r="4" fill={LINE} stroke="#0f172a" strokeWidth="2" /></g>)}
      </svg>
      {h && <div className="pointer-events-none absolute right-1 top-0 rounded bg-slate-800 px-2 py-1 text-[10px] text-slate-200 ring-1 ring-slate-600">{h.t} · {h.kmh} km/h · {h.duty ?? "—"}</div>}
    </div>
  );
}
...
export default function TracePanel({ unit }: { unit: string }) {
  ...
  if (!t) return <p className="text-sm text-slate-500">Loading trace…</p>;
  return (
    <section>
      <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Track &amp; trace · {t.unit} <span className="font-normal normal-case text-slate-500">· {t.pings} pings · {t.source}</span></h2>
      <div className="mb-2 grid grid-cols-4 gap-1.5 text-center">
        {[["Distance", t.distance_km != null ? `${t.distance_km} km` : "—"], ["Moving", fmtH(t.moving_h)], ["Stopped", fmtH(t.stopped_h)], ["Avg / max", `${t.avg_moving_kmh} / ${t.max_kmh}`]].map(([l, v]) => (
          <div key={l} className="rounded bg-slate-950/60 p-2 ring-1 ring-slate-800"><div className="text-[10px] uppercase text-slate-500">{l}</div><div className="text-sm font-semibold tabular-nums text-slate-100">{v}</div></div>
        ))}
      </div>
      <div className="rounded bg-slate-950/60 p-2 ring-1 ring-slate-800">
        <div className="mb-1 text-[10px] uppercase text-slate-500">Speed, km/h · {t.window[0]?.slice(11, 16)}–{t.window[1]?.slice(11, 16)}</div>
        <Sparkline s={t.speed_series} />
      </div>
      {t.stops.length > 0 && (
        <table className="mt-2 w-full text-[11px]">
          <thead className="text-slate-500"><tr><th className="text-left font-normal">Stop</th><th className="text-right font-normal">In</th><th className="text-right font-normal">Out</th><th className="text-right font-normal">Dwell</th><th className="text-right font-normal">Billable</th></tr></thead>
          <tbody>{t.stops.map((s) => (
            <tr key={s.visit_id} className="border-t border-slate-800"><td className="py-1">{s.facility} <span className="text-slate-500">· {s.stop_kind} · {s.state.replace(/_/g, " ")}</span></td>
