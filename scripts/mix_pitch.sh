...
#
#   scripts/mix_pitch.sh  →  .demo-logs/pitch/dockrisk-pitch-rehearsal.mp4
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="$ROOT/.demo-logs/pitch"
TTS="$ROOT/.demo-logs/tts-pitch"
SCRIPT="$ROOT/docs/demo/explainer/pitch-ko.md"
FINAL="$OUT/dockrisk-pitch-rehearsal.mp4"
...
for line in script.read_text(encoding="utf-8").splitlines():
    ...
    if num and line.strip().startswith("EN:"):
        say[num].append(line.strip()[3:].strip())
...
def render(text, path):
    ...
    d.text((44, 20), "SAY THIS", font=SMALL, fill=DIM)
    if text:
...
printf '==> %s\n' "$FINAL"
python3 -c "print(f'    {int($D//60)}m {int($D%60):02d}s · {$S/1e6:.0f} MB · 무대에서 말할 영어가 하단 밴드에')"
