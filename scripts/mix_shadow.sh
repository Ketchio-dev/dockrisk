...
#
#   scripts/mix_shadow.sh →  .demo-logs/shadow/dockrisk-shadow-run.mp4
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
OUT="$ROOT/.demo-logs/shadow"
TTS="$ROOT/.demo-logs/tts-shadow"
SCRIPT="$ROOT/docs/demo/explainer/shadow-en.md"
FINAL="$OUT/dockrisk-shadow-run.mp4"
BAND=190          # taller than a caption band: this is meant to be read, not glanced at
PIC=$((1080 - BAND))

[[ -f "$OUT/picture.mp4" ]] || { echo "no picture cut — run scripts/record_shadow.sh first" >&2; exit 1; }
...
for line in script.read_text(encoding="utf-8").splitlines():
    ...
    if num and line.startswith(">"):
        t = line.lstrip("> ").strip()
        if t:
            # one sentence per cue: he is reading along, so a cue should end where he breathes
            import re as _re
            for part in _re.split(r"(?<=[.!?])\s+", t):
                if part.strip(): say[num].append(part.strip())
...
for numstr, _name, lenstr in rows:
    ...
    if lines:
        ...
        for l in lines:
            ...
            t += d
    clock += span
(out / "teleprompter.srt").write_text("\n".join(srt), encoding="utf-8")
meta = [";FFMETADATA1"]
...
def render(text, path):
    ...
    d.text((44, 18), "SPEAK ALONG", font=SMALL, fill=DIM)
    if text:
        lines = wrap(d, text, W - 200, BIG); lh = 58
        y = (band - lh * len(lines)) / 2 + 10
        for ln in lines:
...
printf '==> %s\n' "$FINAL"
python3 -c "print(f'    {int($D//60)}m {int($D%60):02d}s · {$S/1e6:.0f} MB · 네 목소리로 말하는 대본 · 따라 말하면 된다')"
